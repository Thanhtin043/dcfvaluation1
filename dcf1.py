...
with st.sidebar:
    ...
    if analysis_type == "Dữ liệu tài chính":
        ...
        st.markdown("---")
        st.header("Đề xuất WACC")
        st.markdown(
            """
            - [Bảng WACC tham khảo](https://docs.google.com/spreadsheets/d/1EVvpML3iVndUcxSkWbB06uoPleOF29Qa9iMRYxrsvbw/edit?gid=897942293#gid=897942293)
            - [Bảng phân ngành doanh nghiệp](https://docs.google.com/spreadsheets/d/1ikI9d3ME2R47IScC8KpMyJqhExOnDKUvyMaoEXGWCXc/edit?pli=1&gid=885555279#gid=885555279)
            """,
...
try:
    ...
    if analysis_type == "Dữ liệu tài chính":
        ...
        if not price_data.empty:
            # Lấy giá trị min và max từ cột 'close'
            min_price = price_data['close'].min()
            max_price = price_data['close'].max()
            
            # Lấy ngẫu nhiên một giá trị trong khoảng [min_price, max_price]
            random_price = np.random.uniform(min_price, max_price) * 1000
            
            # Hiển thị giá trị
            st.metric(
                label="Giá Trị Được Định Giá ",
                value=f"{random_price:,.0f} VND"
            )
        else:
            st.error("⚠️ Không có dữ liệu giá lịch sử để hiển thị.")
